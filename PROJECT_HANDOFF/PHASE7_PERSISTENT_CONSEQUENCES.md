...
**Active planning / next implementation: Phase 7A — Persistent Consequence Foundation.**

Certified starting baseline: GitHub Actions Run #106 (`34730041125`), expanded source `5e8fd2cd51199b66e1239ecdd0fecf527f49c6c6`, package `everthread-life-unwritten@0.12.0`, save schema 12. Phase 6 is closed.
...
The post-6C Financial Pressure correction exposed the architectural pressure directly: FinanceSystem currently has to notice already-due delayed stories so it does not steal the single pending-event slot. Phase 7A should remove that responsibility from feature systems.
...
Phase 7A is expected to advance save schema **12 → 13** because persistent scheduler/cooldown metadata is new durable state.
