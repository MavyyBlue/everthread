...
**Current approved direction:** dedicated location-scene migration. Runs #187/#189/#190 establish the reusable shell and player-hardened interaction contract; Run #192 widens that certified set to **Weaver Park + Threadtone Music Studio + Central Everthread Bank**. The immediate gate is player-facing Bank review, not a fourth migration. After approval, select exactly one additional Astra location, map its semantic objects to existing owners, preserve map state and fallback/action parity, inherit the shared drawer/cover geometry contract, and repeat connected regression + full CI certification. Do not bulk-import remaining art simply because it exists. Character Visual remains a separate approved direction with its Run #170–#185 foundations preserved; future Yuki age-stage art can still plug into that contract later. Feedback is current at **5 total / 0 unresolved by triage** through Run #192. No Phase 11 label is implied.
