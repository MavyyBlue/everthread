...
Core simulation uses seeded RNG, state-scoped `makeStateId`, and no `Math.random()` for simulation state. Yearly processors that can award money, advance contracts, resolve projects/seasons, or create incidents must be idempotent per age. Current **certified** save schema is **18**. Newest certified repository/gameplay source is Run #238 / `1f3786df83a37a4756a2da670566b0ceeb5fc2b3`; QA-4 infrastructure ownership remains the closed Run #201 wall. Historical schema-14 Phase 7C owns bounded durable world-condition lifecycle state; Phase 7 remains closed. Phase 7A introduced durable consequence-scheduler cooldown/history authority through shared `CURRENT_SAVE_VERSION` and is certified/deployed. Phase 6A advanced to schema 11 for durable revolving-credit account/history authority; Phase 6B3 advanced to schema 12 for durable auto-pay, card past-due, and secured paid-ahead payment state. Schema increments are justified by new durable authority, not by presentation-only projections or bounded primitive flags.
...
## QA orchestration and certification authority

Run #194 certifies QA-1 bounded preflight/evidence; Runs #196–#197 certify QA-2 compiler-graph/build cleanup; Run #199 certifies QA-3 registry/timing; Run #201 certifies QA-4 bounded process isolation. QA tooling is infrastructure around the proof; it is never permission to weaken the proof.

- GitHub Actions remains the final certification authority. `npm run preflight` is the required standard aggregate and `npm run preflight:deep` remains the deep aggregate. A bounded stage pass is development evidence only and must never be described as a certified slice.
- Standard canonical preflight now owns six explicit responsibilities in order: **Engine TypeScript → Test TypeScript → App TypeScript → Node/Vite Config TypeScript → complete regression wall → production build**. Failure of any mandatory stage prevents certification, certified baseline packaging, and deployment.
- `src/tests/regressionRegistry.ts` remains the authoritative registry for specialized suites and `src/tests/regressionMetadata.ts` owns core-case execution classification only. `src/tests/runRegression.ts` executes exactly one selected lane per process; `scripts/run-regression-wall.mjs` owns the parent wall, launches at most two isolated Node workers, verifies the exact union/disjointness of case indexes and suite IDs, then runs the established auxiliary wall. Registry/partition metadata is QA evidence only and must not become simulation state.
- QA timing output is diagnostic evidence, not permission to skip slow tests. `CASE_TIMING`, `SUITE_TIMING`, `SHARD_TIMING`, `SHARD_COVERAGE`, and `WALL_TIMING` must preserve pass/fail truth. QA-4 requires exact coverage partition (`82` core cases and `77` specialized suites, no overlap) before the parent wall may pass; one-worker process-isolated fallback must remain available for debugging.
- `tsconfig.app.json` must not root `src/tests`; test coverage belongs to `tsconfig.tests.json`. The engine remains separately checked because its compiler context intentionally constrains engine/browser coupling. `tsconfig.node.json` owns Vite/config TypeScript. Do not collapse these responsibilities merely to reduce elapsed time.
- `npm run build` is now a **bundle/build-info responsibility**, not a hidden second compiler wall: `vite build && node scripts/write-build-info.mjs dist/build-info.json`. TypeScript safety must remain explicit in the preceding canonical stages. A future standalone build invocation is therefore not equivalent to canonical preflight certification.
- Bounded stages (`preflight:engine`, `preflight:tests-ts`, `preflight:app`, `preflight:node`, `preflight:regressions`, `preflight:build`, `preflight:content`, `preflight:sim`) exist so constrained development tools can validate one gate at a time without holding a single long-lived process open. They do not change required assertions or test parameters.
- `.everthread/preflight-report.json` binds evidence to exact source state and records selected stage, Node/platform, per-stage timing/status, and relevant package/lock/config SHA-256 inputs. Clean-source evidence may be reusable in future cache/resume work; dirty-tree evidence remains diagnostic only.
- QA-2's measured build-stage reduction is **10,342 → 4,233 ms** between certified Runs #196 and #197. Treat this as a build-stage result only; independent regression/typecheck timing can move total wall time in either direction.
- Optimization sequencing is now complete: **QA-1 stageability/evidence → QA-2 compiler graph/build duplication → QA-3 registry/timing → QA-4 measured process isolation**, all certified. Stop here unless new measured evidence justifies reopening QA infrastructure; do not manufacture QA-5 work. Never combine test-body rewrites, coverage changes, and concurrency changes into one certification slice.
- QA-3 identified the core 25-life smoke and Integrated Long-Life as the dominant heavy work. Certified Run #201 isolates only that heavy work into a separate Node process while preserving the standard lane and exact full-wall coverage. The parent starts at most two workers; browser globals/module fixtures stay process-local; no mutable `GameState` is shared. Run #201 coverage handshake is `core=81+1/82 specialized=76+1/77 overlap=0`, and the five-stage wall is 20,544 ms versus 35,727 ms in adjacent Run #200. Never reduce long-life counts, seeds, age caps, assertions, or use affected-tests-only certification.
...
A special presentation surface must never become a second simulation for the person it displays. Run #178 establishes this contract for the secret-origin Yuki NPC; Run #185 hardens the provenance boundary after a real long-running save exposed bounded-memory eviction.
...
- Durable special identity is provenance, not a name check. The existing `state.flags['secretCode:yuki:9426']` pointer is the primary authoritative provenance because it survives bounded narrative-history pruning. The `secret_yuki_9426` origin memory remains valuable story/history and a legacy-recovery fallback, but it must not be the sole live routing key. An unrelated NPC named Yuki or copied visual fields must not gain special routing.
- General rule: never make durable identity, ownership, lineage, or routing depend exclusively on a bounded narrative/history collection. Bounded histories are projections/evidence; durable authority must live in the owning stable state record or pointer.
...
Runs #187, #189, and #190 establish and player-harden the reusable dedicated-location presentation contract with Weaver Park and Threadtone Music Studio; Run #192 proves the same contract can widen to Central Everthread Bank without moving finance authority into scene UI. This is a UI/navigation layer over existing town and gameplay authorities, not a second place/action simulation.
...
- Run #192 extends that rule to finance. `BankLocationPanel` is presentation routing only. `MoneySummaryView`, `InvestmentMarketView`, and `CreditBankingFocusedView` may be reused across Bank/Assets/Credit UI, but authoritative balances, liabilities, credit accounts/scores, payment obligations, personal borrowing, investments, underwriting, and mutations remain in their existing systems and `GameEngine` actions. A scene must never create a bank-local balance, debt ledger, credit profile, portfolio, or direct cash mutation. Scene-level bank availability may expose a read-only/focused panel while the owning view/action retains its own age/action gate (for example, investments remain locked before 18).
- Scene art is presentation, never collision/gameplay authority. Authored hotspot rectangles are normalized against the declared **1024×1536** canvas. After player review in Run #189, the shared shell uses one `coverLocationScene` transform for artwork, semantic hotspots, and prop composition so full-bleed art can touch viewport edges without detaching interaction geometry. Extreme short-landscape crop risk is covered by the parity-preserving Things-to-do fallback. Props may use recorded alpha bounds without changing interaction semantics.
- Each semantic target must remain thumb-safe (minimum 48px effective target), while visible labels may use independent readable widths/edge anchoring. Transparent controls still require visible focus/press affordances and outlined/shadowed copy over art. Numbered hotspot markers are not sequencing authority; the shared hand/interact affordance communicates actionability. Every hotspot action must also be reachable through the first-class **Things to do** fallback; accessibility/fallback paths must call the exact same action bindings.
- Back navigation is local-stack aware: detail/picker/confirmation → object menu → scene → preserved map. Do not let Android/system Back bypass unresolved local UI or remount the map unnecessarily. The bottom utility surface is one presentation-only drawer: handle + Things to do + Map move as one unit; expanded/collapsed/hidden state stays local React UI state and never enters `GameState` or saves.
...
- Run #187 ships Weaver Park + Threadtone Music Studio and four runtime assets; Runs #189–#190 refine the shared shell and unified drawer; Run #192 adds Central Everthread Bank; the incremental chain now reaches **Everthread College Location #15** through Run #238. Certified totals are **15 scenes / 46 semantic groups / 89 bindings / 28 selected Astra runtime assets**. Location Scene regression is **205/205**. Run #238 advances save schema to **18** only because College campus housing adds legitimate durable ResidentialLife state; the scene itself remains projection/routing UI.

## Campus housing and non-property residence authority

Run #238 establishes the first durable player residence that is not inferred from property ownership/family/rented fallback. Preserve these boundaries:

- `ResidentialLifeSystem` owns optional campus-housing assignment and current-residence projection. The durable record exists because housing choice must survive save/load, rewind, age-up, map projection, and enrollment lifecycle.
- `EducationSystem` owns whether local post-secondary enrollment exists; enrollment ending must invalidate campus housing through the Education → ResidentialLife lifecycle seam. Education does not own a separate dorm ledger.
- `PropertySystem` still owns purchased/inherited property and primary-property-home choice. A dorm is never inserted into `assets.properties`, never gets market value/condition/rental/mortgage fields, and never participates in estate equity. Setting an owned property as home exits campus housing through the shared residence handshake.
- Campus housing has no independent rent, mortgage, loan, or annual charge. Its cost is intentionally represented by existing education costs; never add a second dorm financing/debt authority unless Mavyy explicitly changes the product model.
- `playerResidenceProjection` remains the one read model used by Living Map/residential UI. While dorming, College may be the current home marker while owned Threadwell property remains a distinct asset/property context. Incarceration retains higher-priority institutional residence.
- Schema 18 migration creates empty campus-housing state for older saves deterministically/idempotently and consumes no gameplay RNG or runtime IDs. Descendant continuation must not copy a predecessor's dorm assignment.
