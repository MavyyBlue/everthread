...
Core simulation uses seeded RNG, state-scoped `makeStateId`, and no `Math.random()` for simulation state. Yearly processors that can award money, advance contracts, resolve projects/seasons, or create incidents must be idempotent per age. Current **certified** save schema is **17**. Newest certified repository/QA source is Run #199 / `7c8d0f2fc9da1a4461877caf1ae9a2b4b1660c28`; newest gameplay implementation remains Run #192 / `1f5c8d598b6f277f26ffda5d7683d7474141200e`. Historical schema-14 Phase 7C owns bounded durable world-condition lifecycle state; Phase 7 remains closed. Phase 7A introduced durable consequence-scheduler cooldown/history authority through shared `CURRENT_SAVE_VERSION` and is certified/deployed. Phase 6A advanced to schema 11 for durable revolving-credit account/history authority; Phase 6B3 advanced to schema 12 for durable auto-pay, card past-due, and secured paid-ahead payment state. Schema increments are justified by new durable authority, not by presentation-only projections or bounded primitive flags.
...
Run #194 certifies QA-1 bounded preflight/evidence. Runs #196–#197 certify QA-2 compiler-graph/build cleanup. QA tooling is infrastructure around the proof; it is never permission to weaken the proof.
...
- `src/tests/regressionRegistry.ts` is the authoritative ordered registry for the existing specialized regression suites; `src/tests/runRegression.ts` executes the core suite plus those registered suites serially, and `scripts/run-regression-wall.mjs` owns the five-part process wall. Registry metadata is QA evidence only and must not become simulation state.
- QA-3 timing output is diagnostic evidence, not permission to skip slow tests. `CASE_TIMING`, `SUITE_TIMING`, and `WALL_TIMING` must preserve pass/fail truth and make slow/failing work visible; any future sharding must aggregate failures and keep canonical full-wall coverage.
...
- Optimization sequencing remains preservation-first: **QA-1 stageability/evidence (done) → QA-2 compiler graph/build duplication (done) → QA-3 regression registry/timing (done) → QA-4 measured, proven-isolated process sharding**. Never combine test-body rewrites, coverage changes, and concurrency changes into one certification slice.
- QA-3 preserves existing test bodies and serial order while making core-case, suite, and top-level wall timing inspectable. Certified Run #199 identifies the core 25-life smoke and Integrated Long-Life as the dominant heavy work. QA-4 must prefer separate Node processes over concurrent functions in one VM because some regression files install browser globals or other process-level fixtures. Never share mutable `GameState` across concurrent workers, reduce long-life counts, or use affected-tests-only certification.
