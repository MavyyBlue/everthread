...
Core simulation uses seeded RNG, state-scoped `makeStateId`, and no `Math.random()` for simulation state. Yearly processors that can award money, advance contracts, resolve projects/seasons, or create incidents must be idempotent per age. Current **certified** save schema is **17**. Newest certified repository/QA source is Run #194 / `fb9cc4fb7c271cb2b37188a11f75663535371767`; newest gameplay implementation remains Run #192 / `1f5c8d598b6f277f26ffda5d7683d7474141200e`. Historical schema-14 Phase 7C owns bounded durable world-condition lifecycle state; Phase 7 remains closed. Phase 7A introduced durable consequence-scheduler cooldown/history authority through shared `CURRENT_SAVE_VERSION` and is certified/deployed. Phase 6A advanced to schema 11 for durable revolving-credit account/history authority; Phase 6B3 advanced to schema 12 for durable auto-pay, card past-due, and secured paid-ahead payment state. Schema increments are justified by new durable authority, not by presentation-only projections or bounded primitive flags.
...
Run #194 / `fb9cc4fb7c271cb2b37188a11f75663535371767` certifies QA-1: bounded preflight execution and stronger evidence without reducing the canonical wall. QA tooling is infrastructure around the proof; it is not permission to weaken the proof.

- GitHub Actions remains the final certification authority. `npm run preflight` remains the required standard aggregate; `npm run preflight:deep` remains the deep aggregate. A bounded stage pass is useful development evidence only and must never be described as a certified slice.
- Bounded stages (`preflight:engine`, `preflight:tests-ts`, `preflight:regressions`, `preflight:build`, `preflight:content`, `preflight:sim`) exist so constrained development tools can validate one gate at a time without holding a single long-lived process open. They do not change the required assertions or test parameters.
- `.everthread/preflight-report.json` records exact commit, working-tree cleanliness/fingerprint, selected stage, Node/platform, per-stage timestamps/duration/exit status, and relevant package/lock/config SHA-256 inputs. Clean-source evidence may be marked reusable for future cache/resume design; dirty-tree evidence is diagnostic only. Run #194 does **not** implement resume/caching yet.
- Standard aggregate execution remains fail-fast at stage boundaries. Failure of any mandatory stage prevents certification, certified baseline packaging, and deployment.
- Optimization sequencing is preservation-first: **QA-1 stageability/evidence → QA-2 compiler graph cleanup → regression registry/timing → proven-isolated process sharding**. Never combine a test-body rewrite, compiler-coverage change, and concurrency change into one certification slice.
- QA-2 must preserve distinct Engine/Test/App TypeScript responsibilities. The production app project may stop rooting test files only after an explicit app typecheck exists and old/new diagnostics are proven equivalent under the existing certified wall. Do not trade strictness, save/determinism coverage, life-count stress, or regression breadth for elapsed time.
