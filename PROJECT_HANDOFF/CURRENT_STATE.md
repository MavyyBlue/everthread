...
- **Authority contract:** EducationSystem owns programs/enrollment/tuition/aid/study/dropout; SchoolWorldSystem owns persistent classmates/groups/academic records; Shared Experience/Relationship owns social outcomes; CareerSystem/WorkplaceSystem owns College jobs; ResidentialLife owns the durable campus-housing assignment. Location scenes remain presentation/routing only.
- **Dorm contract:** active local post-secondary enrollment may opt into campus housing. Dorm residency is not a property, rental contract, mortgage, financing product, cash ledger, or debt. Owned properties remain owned. `playerResidenceProjection` treats College as the current home while assigned; Living Map therefore moves **You live here** to College while owned Threadwell property remains independently visible. Education lifecycle ends ineligible housing; Property/ResidentialLife handshake exits dorming when an owned property is made home; incarceration remains the higher-priority institutional projection.
- Save schema advances **17 → 18** because campus housing is durable state. Migration is deterministic/idempotent and RNG/runtime-ID neutral; old saves receive empty ResidentialLife campus housing. Rewind, save/load/import/export, old saves, descendant continuation, graduation/dropout, owned-home restoration, property actions, and one-current-residence invariants are regression-covered.
- Certified scene totals are **15 scenes / 46 semantic groups / 89 scene action bindings / 28 selected Astra runtime assets**. Shared Experience now includes the College social activity without a second relationship state.
