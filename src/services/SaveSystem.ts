...
import { migrateConsequenceSchedulerState } from '../systems/ConsequenceSystem';
import { migrateWorldConditionState } from '../systems/WorldConditionSystem';
import { CURRENT_SAVE_VERSION } from '../core/saveVersion';
import { migrateEverthreadSetting, normalizeEverthreadResidence } from '../systems/SettingSystem';
import { migratePersonalInventoryState } from '../systems/PersonalInventorySystem';
import { normalizeCharacterAppearance } from '../systems/CharacterVisualSystem';
import { migrateResidentialLifeState } from '../systems/ResidentialLifeSystem';
import { normalizeNpcVisualState } from '../systems/NpcVisualSystem';
import { normalizeSecretYukiState } from '../systems/SecretCodeSystem';
import { migrateNpcPreferenceState, normalizeNpcPreferenceState } from '../systems/NpcPreferenceSystem';
...
export function migrateSave(raw:unknown):GameState {
  ...
  if(version<8){
    state.employment.partTimeJobs=state.employment.partTimeJobs??[];
    state.employment.partTimeHistory=state.employment.partTimeHistory??[];
  ...
  if(version<11){
    migrateCreditState(state);
    version=11;
  }
  if(version<12){
    migratePaymentState(state);
    version=12;
  }
  if(version<13){
    migrateConsequenceSchedulerState(state);
    version=13;
  }
  if(version<14){
    migrateWorldConditionState(state);
    version=14;
  }
  if(version<15){
  ...
  if(version<17){
    migrateNpcPreferenceState(state);
    version=17;
  }
  if(version<18){
    migrateResidentialLifeState(state);
    version=18;
  }
