...
export async function runEventCoherenceRegression(){
  ...
  verify(procedural.every(event=>event.descriptions.length===3&&new Set(event.descriptions).size===3),'every procedural event must keep three distinct narrative variants without changing description-selection RNG shape');
  verify(procedural.every(event=>event.descriptions.every(description=>description.trim().length>=12&&/[.!?]$/.test(description))),'every procedural narrative variant must remain readable and sentence-complete');
  const brokenNarrativeLead=/^(?:While (?:at|before|after|during|in|on|over|near|outside|inside|behind|beside|ten minutes|late at|while|the |a )|You are (?:at|before|after|during|in|on|over|near|outside|inside|behind|beside|ten minutes|late at|while|the |a ))/i;
  verify(procedural.every(event=>event.descriptions.every(description=>!brokenNarrativeLead.test(description)&&!description.includes('uncomfortable amount of eye contact'))),'procedural descriptions must not use the legacy grammar-breaking setting frames or unrelated eye-contact filler');
  const missingToyNarrative=eventById['childhood_the_missing_toy_1'];verify(missingToyNarrative.descriptions.some(description=>description.startsWith('At the playground,'))&&missingToyNarrative.descriptions.some(description=>description.endsWith('at the playground.')),'childhood scene descriptions must turn raw locations into grammatical context');
  const quizNarrative=eventById['school_the_surprise_quiz_1'];verify(quizNarrative.descriptions.some(description=>description.startsWith('Before a big test,'))&&quizNarrative.descriptions.some(description=>description.endsWith('before a big test.')),'school descriptions must compose timing phrases without duplicated conjunctions');
  ...
  verify(checks===76,'the focused event-coherence suite must retain its intentional 77-check contract');
  return checks;
}
