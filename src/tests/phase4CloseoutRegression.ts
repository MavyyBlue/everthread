...
export async function runPhase4CloseoutRegression(){
  ...
  const serialized=JSON.stringify(coexist);const restored=migrateSave(JSON.parse(serialized) as unknown);verify(restored.saveVersion===SAVE_VERSION&&SAVE_VERSION===12,'Phase 4 closeout remains intact under current save schema 12');
