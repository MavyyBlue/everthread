...
export function runPhase8BTownMapRegression(){
  ...
  verify(state.saveVersion===CURRENT_SAVE_VERSION&&CURRENT_SAVE_VERSION===18,'13 Phase 8B map browsing must remain save-neutral under current schema 18');
  ...
  verify(state.saveVersion===18,'35 browsing Everthread from an out-of-setting fixture must not alter save schema or location authority');
