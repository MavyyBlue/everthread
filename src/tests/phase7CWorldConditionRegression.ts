...
export function runPhase7CWorldConditionRegression(){
  ...
  check(fresh.saveVersion===17&&fresh.worldConditions.active.length===0&&fresh.worldConditions.history.length===0,'05 new games must initialize schema 17 with an empty bounded world-condition state');
  ...
  check(migrated.saveVersion===17&&migrated.worldConditions.active.length===0&&migrated.worldConditions.history.length===0,'06 schema-13 migration must add empty world-condition state without retroactive history');
  ...
  check(roundTrip.saveVersion===17&&validateState(roundTrip).length===0,'39 restored Phase 7C state must validate on schema 17');
