...
export function runNpcHealthMortalityRegression(){
  ...
  verify(restored.saveVersion===18&&alive(restored,importedNpc.id)&&health(restored,importedNpc.id)===1,'current-schema load should repair legacy living zero health without a schema bump');
