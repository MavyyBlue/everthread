...
export function runNpcHealthMortalityRegression(){
  ...
  verify(restored.saveVersion===9&&alive(restored,importedNpc.id)&&health(restored,importedNpc.id)===1,'schema-9 load should repair legacy living zero health without a schema bump');
  verify(!validateState(restored).some(error=>error.includes('alive with terminal health')),'repaired schema-9 save should validate without a living-terminal-health contradiction');
