...
export function runNpcAssetOwnershipRegression(){
  ...
  verify(fresh.saveVersion===12&&SAVE_VERSION===12,'new lives and save service use current schema 12');
  ...
  verify(migrated.saveVersion===12,'v9 saves migrate to current schema 12');
  ...
  const remigrated=migrateSave(migrated);verify(JSON.stringify(remigrated.npcs[legacyNpc.id]!.assetPortfolio)===JSON.stringify(migratedPortfolio),'current-schema migration is idempotent for explicit NPC holdings');
