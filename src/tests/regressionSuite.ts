...
export const regressionCases:RegressionCase[]=[
  ...
  },
  {
    ...
    run:()=>{
      ...
      const migrated=migrateSave(legacy);equal(migrated.saveVersion,18,'save did not migrate to current schema 18');assert(Number.isFinite(migrated.idCounter)&&migrated.idCounter>=10000,'v3→v4 migration did not initialize deterministic id counter');assert(migrated.travel,'travel state missing after migration');assert(migrated.inheritance,'inheritance state missing after migration');assert(migrated.familyPlanning,'family-planning state missing after migration');assert(migrated.actionLedger,'action ledger missing after migration');assert(migrated.socialWorlds,'social-world state missing after migration');equal(validateState(migrated).length,0,'migrated save violates invariants');
    }
  },
  {
    name:'v5 migration preserves consumed yearly actions in the central ledger',
    run:()=>{
      const legacy=highStatAdult('action-ledger-migration');legacy.saveVersion=5;legacy.flags.lastWorkHarderAge=legacy.character.age;legacy.flags.lastRaiseRequestAge=legacy.character.age;delete (legacy as unknown as {actionLedger?:unknown}).actionLedger;const migrated=migrateSave(legacy);equal(migrated.saveVersion,18,'v5 save did not migrate to current schema');equal(actionUsesThisAge(migrated,'career.work_harder'),1,'legacy work-effort use was lost');equal(actionUsesThisAge(migrated,'career.raise'),1,'legacy raise use was lost');equal(actionAllowed(migrated,{policy:'career.work_harder'}),false,'migrated save allowed a duplicate yearly work action');
    }
  },
  {
  ...
  },
  {
    ...
    run:()=>{
      ...
      const a=migrateSave(legacy),b=migrateSave(legacy);equal(a.saveVersion,18,'v8 save did not migrate to current schema');equal(a.rngCounter,beforeCounter,'NPC life migration consumed the player RNG stream');
