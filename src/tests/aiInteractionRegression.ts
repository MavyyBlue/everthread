...
export async function runAiInteractionRegression(){
  ...
  workingState.socialWorlds.push(aiSchoolWorld,aiWorkWorld,aiPartWorld);workingState.businesses.push({id:'ai-working-business',industryId:'software',name:'Blue Loom Labs',foundedAge:27,origin:'inherited',inheritedFromNpcId:'ai-prior-generation',countryId:'everthread',city:'Everthread',capital:200_000,revenue:100_000,expenses:70_000,profit:30_000,employees:5,demand:60,reputation:58,valuation:350_000,productIds:['software_product_1'],priceIndex:1,marketingBudget:5_000,compensationIndex:1,bankrupt:false});workingState.timeline.push({id:'ai-place-memory',year:2069,age:29,category:'relationship',placeId:'weaver-park',importance:3,text:'A defining day at Weaver Park.'});
  ...
  await withEverthreadAiTestbench({state:workingState,screen:'career'},async workingBench=>{
    ...
    const semanticBusiness=(assets.data.businesses as Array<{id:string;origin?:string;inheritedFromNpcId?:string}>).find(item=>item.id==='ai-working-business');verify(semanticBusiness?.origin==='inherited'&&semanticBusiness.inheritedFromNpcId==='ai-prior-generation','79 Assets semantic observation must expose authoritative inherited-business provenance without a shadow family-business record');
    const legacy=assets.data.placeLegacy as {familyLandmarks:number;places:Array<{placeId:string;familyBusinesses:number;currentLifeMilestones:number}>};verify(legacy.familyLandmarks===1&&legacy.places.some(place=>place.placeId==='loomworks-business-district'&&place.familyBusinesses===1),'80 Assets semantic observation must expose the same surviving family-business place projection available to player surfaces');
    verify(legacy.places.some(place=>place.placeId==='weaver-park'&&place.currentLifeMilestones===1),'81 semantic Place Legacy must expose meaningful timeline place context without requiring Map presentation state');
    const semanticBefore=JSON.stringify(workingBench.getState());workingBench.observe('assets');verify(JSON.stringify(workingBench.getState())===semanticBefore,'82 generational place semantic observation must remain read-only');
  });
