...
const seedPrefix=process.argv[5]||undefined;
const report=runSimulation({lives,mode,policy,...(seedPrefix?{seedPrefix}: {})});
console.log(`${formatSimulationReport(report)}\nMode: ${mode} | Policy: ${policy}${seedPrefix?` | Seed prefix: ${seedPrefix}`:''}`);
