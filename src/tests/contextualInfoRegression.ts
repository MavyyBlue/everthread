import { careerInfluenceSnapshots, contextualInfoScale } from '../components/ContextualInfoButton';
...
export function runContextualInfoRegression(){
  ...
  verify(contextualInfoScale(300,420)===1,'1 content that already fits must stay at normal text scale');
  const compact=contextualInfoScale(600,420);
  verify(compact<1&&compact>.6,'2 oversized contextual information must calculate a proportional smaller text scale');
  verify(contextualInfoScale(4000,180)===.46,'3 extreme content must respect the bounded minimum auto-fit scale instead of returning an invalid size');

  const empty=createNewGame({seed:'context-info-empty'});
  verify(careerInfluenceSnapshots(empty).length===0,'4 the developer projection must stay empty without an active special Career World');
  ...
  verify(projected.length===1&&projected[0]?.worldId===world.id,'5 the UI projection must expose the exact active Career World');
  verify((projected[0]?.leaderSupport??0)>75&&(projected[0]?.rivalPressure??0)>70,'6 the UI projection must expose meaningful live leader and rival inputs');
  verify(projected[0]?.processed===false,'7 influence must be labeled as projected before the annual influence processor runs');
  ...
  verify(processed[0]?.processed===true&&processed[0]?.opportunityModifier===stored,'8 after processing, the UI must show the exact stored opportunity modifier rather than silently recomputing another result');
