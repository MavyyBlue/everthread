...
export function runPhase8DPlayerProfileInventoryRegression(){
  ...
  verify(CURRENT_SAVE_VERSION===18&&fresh.saveVersion===18,'01 Phase 8D inventory remains compatible with current schema 18');
  ...
  verify(migrated.saveVersion===18&&migrated.personalInventory.items.length===0,'03 schema-15 saves must migrate to an empty personal inventory');
