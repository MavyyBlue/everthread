...
export function runPaymentAssetManagementRegression(){
  ...
  verify(SAVE_VERSION===17&&fresh.saveVersion===17,'Phase 6B3 payment state remains valid under current schema 17');
  ...
  verify(migrated.saveVersion===17,'schema-11 saves migrate to current schema 17');
  ...
  const remigrated=migrateSave(structuredClone(migrated));verify(JSON.stringify(remigrated)===JSON.stringify(migrated),'current-schema normalization is idempotent');
  ...
  verify(restored.saveVersion===17&&restoredCard.autoPay===false&&restoredCard.pastDueAmount===40,'current-schema save roundtrip preserves card payment preferences and arrears');
