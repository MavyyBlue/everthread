...
export function runPaymentAssetManagementRegression(){
  ...
  verify(SAVE_VERSION===18&&fresh.saveVersion===18,'Phase 6B3 payment state remains valid under current schema 18');
  ...
  verify(migrated.saveVersion===18,'schema-11 saves migrate to current schema 18');
  ...
  verify(restored.saveVersion===18&&restoredCard.autoPay===false&&restoredCard.pastDueAmount===40,'current-schema save roundtrip preserves card payment preferences and arrears');
