...
export function runPhase10DLivingMapProjectionRegression(){
  ...
  const mappedWork=state('10d-mapped-work');mappedWork.socialWorlds.push(workWorld('food-work','Food Service'));const mappedWorkProjection=livingMapProjection(mappedWork);verify(livingMapPlaceContext(mappedWorkProjection,'nightjar-diner')?.contexts.some(item=>item.kind==='work'&&item.sourceIds.includes('food-work'))===true,'16 an implemented venue mapping must project Food Service work onto the real Nightjar Diner marker');
  verify(!livingMapDistrictContext(mappedWorkProjection,'market-row')?.contexts.some(item=>item.sourceIds.includes('food-work')),'17 exact mapped work must not duplicate itself as a vague Market Row district context');
  mappedWork.socialWorlds.push(workWorld('food-part','Coffee & Bakery','part_time'));const multiMappedWork=livingMapProjection(mappedWork);verify(livingMapPlaceContext(multiMappedWork,'nightjar-diner')?.contexts.find(item=>item.kind==='work')?.count===2,'18 multiple mapped workplaces at one real venue aggregate into one bounded place context');
  ...
  const abroad=state('10d-abroad');abroad.assets.properties.push(property({id:'everthread-property'}));abroad.character.countryId='jp';abroad.character.city='Tokyo';const abroadProjection=livingMapProjection(abroad);const abroadThreadwell=livingMapPlaceContext(abroadProjection,'threadwell-residential');verify(!abroadThreadwell?.contexts.some(item=>item.kind==='home'),'40 an out-of-setting fixture removes You live here because residence authority is external');
  verify(abroadThreadwell?.contexts.some(item=>item.kind==='property')===true,'41 a retained Everthread property remains visibly owned in an external projection fixture without rewriting the player location');
