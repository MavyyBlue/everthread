...
export function runPhase10BWorkingEverthreadRegression(){
  ...
  const technology=workplaceWorldLocation(workWorld('work-tech','Technology'));verify(technology?.districtId==='eastworks'&&technology.anchorPlaceId==='loomworks-business-district','15 unmapped/technology ordinary work must deterministically fall back to Eastworks / business district');
  ...
  const originalBusinessLocation=JSON.stringify(localLocation);founded.character.countryId='us';founded.character.city='Seattle';verify(JSON.stringify(businessWorkLocation(founded,localBusiness))===originalBusinessLocation,'24 a founded company must not teleport when the player emigrates after founding it');
  verify(localBusiness.countryId===EVERTHREAD_COUNTRY_ID&&localBusiness.city===EVERTHREAD_CITY,'25 relocation must not rewrite business-owned location provenance');
