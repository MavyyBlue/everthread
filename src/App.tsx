...
import { loadGame, loadSettings, saveGame } from './services/SaveSystem';
...
export default function App(){const state=useGameState(s=>s);const[tab,setTab]=useState<Tab>('life');const[meta,setMeta]=useState(false);const[newLife,setNewLife]=useState(false);const[toast,setToast]=useState('');const[booted,setBooted]=useState(false);
 useEffect(()=>{void(async()=>{try{const saved=await loadGame('slot-1');if(saved){Object.assign(saved.settings,loadSettings());gameEngine.replaceState(saved);}}finally{setBooted(true);}})();},[]);
 ...
 return <div className="app-shell"><header className="app-bar"><button className="brand-button" onClick={()=>setTab('life')} aria-label="Go to Life"><span className="brand-mark brand-mark--small">E</span><span><strong>Everthread</strong><small>Life Unwritten</small></span></button><button className="icon-button" onClick={()=>setMeta(true)} aria-label="Progress, past lives, and settings">•••</button></header>
  ...
  <EventSheet state={state} onResult={onResult}/><DeathSheet state={state} onResult={onResult} onNewLife={()=>setNewLife(true)}/><MetaSheet open={meta} onClose={()=>setMeta(false)} onNewLife={()=>{setMeta(false);setNewLife(true);}}/><BottomSheet open={newLife} title="Create a new life" onClose={()=>setNewLife(false)} wide><NewLifeForm onCreated={()=>{setNewLife(false);setTab('life');}}/></BottomSheet><Toast message={toast}/>
 </div>;
}
