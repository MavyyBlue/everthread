...
export default function App(){const state=useGameState(s=>s);const[tab,setTab]=useState<Tab>('life');const[meta,setMeta]=useState(false);const[newLife,setNewLife]=useState(false);const[toast,setToast]=useState('');const[booted,setBooted]=useState(false);const[vfxBursts,setVfxBursts]=useState<ActionVfxBurst[]>([]);const[institutionRoute,setInstitutionRoute]=useState<InstitutionRouteRequest>();const[mapFocusPlaceId,setMapFocusPlaceId]=useState<string>();const pressRef=useRef<{x:number;y:number;snapshot:ActionVfxSnapshot;at:number}|undefined>(undefined);const nextVfxId=useRef(1);const routeSerial=useRef(1);
 ...
 const createRandomLife=async()=>{await gameEngine.flushSaves();await saveGame(gameEngine.getState());const slotId=await allocateSaveSlotId();setActiveSaveSlotId(slotId);gameEngine.newLife({slotId});setInstitutionRoute(undefined);setMapFocusPlaceId(undefined);setTab('life');};
 ...
 return <div className="app-shell" onPointerDownCapture={event=>{const target=event.target as Element;if(!target.closest('button'))return;pressRef.current={x:event.clientX,y:event.clientY,snapshot:captureActionVfxSnapshot(gameEngine.getState()),at:performance.now()};}} onKeyDownCapture={event=>{if(event.key!=='Enter'&&event.key!==' ')return;const target=event.target as Element;const button=target.closest('button');if(!button)return;const rect=button.getBoundingClientRect();pressRef.current={x:rect.left+rect.width/2,y:rect.top+rect.height/2,snapshot:captureActionVfxSnapshot(gameEngine.getState()),at:performance.now()};}}><header className="app-bar"><button className="brand-button" onClick={()=>selectTab('life')} aria-label="Go to Life"><img className="brand-icon brand-icon--small" src={officialEverthreadIcon} alt="" aria-hidden="true"/><span><strong>Everthread</strong><small>Life Unwritten</small></span></button><div style={{display:'flex',gap:8}}><ContextualInfoButton tab={tab} state={state}/><button className="icon-button" onClick={()=>setMeta(true)} aria-label="Progress, life saves, and settings">•••</button></div></header>
  <div className="screen-host">{tab==='life'&&<LifeScreen state={state} onResult={onResult} routeRequest={institutionRoute} onReturnToMap={returnToMap}/>} {tab==='people'&&<Suspense fallback={<main className="screen"><div className="empty-card">Opening Threadspace…</div></main>}><PeopleScreen state={state} onResult={onResult}/></Suspense>} {tab==='map'&&<Suspense fallback={<main className="screen"><div className="empty-card">Opening Everthread map…</div></main>}><TownMapScreen state={state} onNavigate={navigateFromMap} initialSelectedId={mapFocusPlaceId}/></Suspense>} {tab==='activities'&&<ActivitiesScreen state={state} onResult={onResult} routeRequest={institutionRoute} onReturnToMap={returnToMap}/>} {tab==='career'&&<CareerScreen state={state} onResult={onResult} routeRequest={institutionRoute} onReturnToMap={returnToMap}/>} {tab==='assets'&&<AssetsScreen state={state} onResult={onResult} routeRequest={institutionRoute} onReturnToMap={returnToMap}/>}</div>
  <nav className="bottom-nav" aria-label="Primary navigation">{tabs.map(([id,label,icon])=><button className={tab===id?'active':''} key={id} onClick={()=>selectTab(id)} aria-current={tab===id?'page':undefined}><span aria-hidden="true">{icon}</span><small>{label}</small></button>)}</nav>
  <ActionVfxLayer bursts={vfxBursts} reducedMotion={state.settings.reducedMotion}/><EventSheet state={state} onResult={onResult}/><DeathSheet key={state.character.id} state={state} onResult={onResult} onNewLife={()=>setNewLife(true)} onRandomLife={()=>void createRandomLife()}/><MetaSheet open={meta} onClose={()=>setMeta(false)} onNewLife={()=>{setMeta(false);setNewLife(true);}} onLifeOpened={()=>{setMeta(false);selectTab('life');}}/><BottomSheet open={newLife} title="Create a new life" onClose={()=>setNewLife(false)} wide><NewLifeForm onCreated={()=>{setNewLife(false);setInstitutionRoute(undefined);setMapFocusPlaceId(undefined);setTab('life');}}/></BottomSheet><Toast message={toast}/>
 </div>;
}
