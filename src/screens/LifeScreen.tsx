...
export function LifeScreen({state,onResult}:{state:GameState;onResult:(r:EngineResult)=>void}){
  ...
  return <main className="screen life-screen">
    ...
    <section className="quick-facts"><div title={exactMoney(netWorthValue)}><small>Net worth</small><strong>{formatMoney(netWorthValue)}</strong></div><div><small>Career</small><strong>{state.employment.current?.title??(state.character.age<18?'Growing up':'Unemployed')}</strong></div><div><small>Fame</small><strong>{Math.round(state.fame.fame)}</strong></div></section>
