...
import { canTryForBiologicalChild, npcReproductiveSex } from '../systems/ReproductionSystem';
...
export function PeopleScreen({state,onResult}:{state:GameState;onResult:(r:EngineResult)=>void}){
  ...
  const biologicalPairAvailable=canTryForBiologicalChild(state,partner?.npcId);
  ...
  const personSheet=<BottomSheet open={!!selected} title={npc?`${npc.firstName} ${npc.lastName}`:'Relationship'} onClose={()=>setSelectedNpcId(undefined)}>{selected&&npc&&<>
    ...
    <div className="sheet-section"><h3>Connections</h3><p className="muted">To you: {selected.type.replaceAll('_',' ')}.</p>{['partner','fiance','spouse'].includes(selected.type)&&<p className="memory">Family planning · reproductive sex: {npcReproductiveSex(state,npc)}.</p>}{npc.partnerId&&state.npcs[npc.partnerId]&&<p className="memory">Partner link: {state.npcs[npc.partnerId]!.firstName} {state.npcs[npc.partnerId]!.lastName}</p>}{npc.parentIds.map(id=>state.npcs[id]).filter(Boolean).map(parent=><p className="memory" key={`parent-${parent!.id}`}>Parent: {parent!.firstName} {parent!.lastName}</p>)}{npc.childIds.map(id=>state.npcs[id]).filter(Boolean).map(child=><p className="memory" key={`child-${child!.id}`}>Child: {child!.firstName} {child!.lastName}</p>)}</div>
  ...
  const workspaceControls=<PeopleWorkspaceActions
    ...
    canAdopt={canAdopt}
    biologicalPairAvailable={biologicalPairAvailable}
...
function PeopleWorkspaceActions({state,onResult,partnerId,expecting,canTryChild,canAdopt,biologicalPairAvailable,newbornPresent}:{state:GameState;onResult:(r:EngineResult)=>void;partnerId?:string;expecting:GameState['familyPlanning']['pregnancy'];canTryChild:boolean;canAdopt:boolean;biologicalPairAvailable:boolean;newbornPresent:boolean}){
  return <div className="threadspace-people-actions">
    ...
    {state.character.age>=18&&<div className="threadspace-filter-section">
      ...
      <p className="threadspace-panel-copy">{expecting?`You are expecting ${expecting.expectedChildren===2?'twins':expecting.expectedChildren===3?'triplets':'a child'} next year.`:partnerId&&!biologicalPairAvailable?'This relationship can grow through adoption. Try for child is available only to pairings that can conceive in Everthread’s current biological-parenting model.':'Children age, form relationships, build careers, and can carry the thread into another generation.'}</p>
      <div className="threadspace-people-action-grid">
        {biologicalPairAvailable&&<button onClick={()=>onResult(gameEngine.haveChild(partnerId,false))} disabled={!partnerId||!!expecting||!canTryChild||newbornPresent}>{expecting?'Expecting':!canTryChild?'Tried this year':'Try for child'}</button>}
        <button onClick={()=>onResult(gameEngine.haveChild(partnerId,true))} disabled={!!expecting||!canAdopt||newbornPresent}>{!canAdopt?'Adopted this year':'Adopt'}</button>
      </div>
    </div>}
  </div>;
}
