...
export function CareerScreen({state,onResult}:{state:GameState;onResult:ActionResultHandler}){
  ...
  const jobs=fullTimeAvailability.allowed?availableJobs(state).filter(j=>`${j.title} ${j.industry}`.toLowerCase().includes(q.toLowerCase())).slice(0,80):[];
  ...
  const partTimeOptions=partTimeAvailability.allowed?partTimeOptionsFor(state):[];
  ...
  return <main className="screen">
    ...
    {tab==='work'&&<>
      ...
      {!fullTimeAvailability.allowed?<div className="empty-card">{fullTimeAvailability.message}</div>:<><SearchField value={q} onChange={setQ} placeholder="Search qualified jobs"/><div className="list-compact">{jobs.map(job=><button key={job.id} disabled={!canStartJob||!actionAllowed(state,[{policy:'career.application.total'},{policy:'career.application.job',target:job.id}])} onClick={()=>onResult(gameEngine.applyForJob(job.id))}><span><strong>{job.title}</strong><small>{job.industry} · {job.salaryRange[0].toLocaleString()}–{job.salaryRange[1].toLocaleString()}</small></span><b>Apply</b></button>)}</div></>}</>}
      {visibility.partTimeWork&&<section className="action-card"><div className="section-heading"><div><p className="eyebrow">Flexible work</p><h2>Part-time jobs</h2></div><span>{partTimeHours}/{partTimeLimit} hrs/week</span></div>{(state.employment.partTimeJobs??[]).map(record=><article className="school-group-card joined" key={`${record.jobId}-${record.startAge}`}><div><strong>{record.title}</strong><small>{record.company} · {record.hoursPerWeek} hrs/week · {formatMoney(record.salary)}/year · performance {Math.round(record.performance)}</small></div><div className="school-group-actions"><button className="secondary-button" onClick={()=>onResult(gameEngine.quitPartTimeJob(record.jobId))}>Quit</button></div></article>)}{!partTimeAvailability.allowed&&<p className="muted">{partTimeAvailability.message}{(state.employment.partTimeJobs??[]).length?' Existing jobs are preserved until you choose to leave them.':''}</p>}{partTimeAvailability.allowed&&partTimeOptions.slice(0,6).map(job=>{const canStart=(state.employment.partTimeJobs??[]).length<3&&partTimeHours+10<=partTimeLimit&&actionAllowed(state,[{policy:'career.part_time.start'},{policy:'career.part_time.job',target:job.id}]);return <article className="school-group-card" key={job.id}><div><strong>{job.title}</strong><small>{job.industry} · 10 hrs/week · about {formatMoney(job.hourlyRate*10*52)}/year before local wage scaling</small></div><div className="school-group-actions"><button disabled={!canStart} onClick={()=>onResult(gameEngine.startPartTimeJob(job.id,10))}>Apply</button></div></article>})}</section>}
...
function SpecialPaths({state,onResult}:{state:GameState;onResult:ActionResultHandler}){
  ...
  return <>
    ...
    <div className="special-paths">
      {specialPathAgeVisible(state,'acting')&&<Path title="Acting" stat={`${actingLife.status} · Skill ${Math.round(Number(state.specialCareers.acting?.skill??0))}`} actions={[
      ...
      {specialPathAgeVisible(state,'sports')&&<Path title="Professional sports" stat={`${String(sportsTrack?.sport??'Choose a sport')} · ${sportsLife.status}${sportsSeasons?` · ${sportsSeasons} season${sportsSeasons===1?'':'s'}`:''}`} actions={[
      ...
      {specialPathAgeVisible(state,'politics')&&<Path title="Politics" stat={`Office level ${Number(state.specialCareers.politics?.office??0)} · approval ${Math.round(Number(state.specialCareers.politics?.approval??0))}`} actions={[
      ...
      {specialPathAgeVisible(state,'military')&&<Path title="Military" stat={`${String(state.specialCareers.military?.branch??'Not enlisted')} · rank ${Number(state.specialCareers.military?.rank??0)}`} actions={[
      ...
      {specialPathAgeVisible(state,'modeling')&&<Path title="Modeling" stat={`${modelingLife.status} · Jobs ${Number(state.specialCareers.modeling?.jobs??0)}`} actions={[
      ...
      {specialPathAgeVisible(state,'racing')&&<Path title="Motorsport" stat={`${racingLife.status} · Skill ${Math.round(Number(state.specialCareers.racing?.skill??0))}`} actions={[
      ...
      {specialPathAgeVisible(state,'directing')&&<Path title="Film directing" stat={`${directingLife.status} · Films ${Number(state.specialCareers.directing?.filmsDirected??0)}`} actions={[
      ...
      {specialPathAgeVisible(state,'crimeOrg')&&<Path title="Organized crime" stat={`${String(state.specialCareers.crimeOrg?.rank??'Not joined')} · standing ${Math.round(Number(state.specialCareers.crimeOrg?.standing??0))}`} actions={[
