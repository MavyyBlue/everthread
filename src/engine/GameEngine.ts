...
export class GameEngine {
  ...
  actingLesson(){return this.run(()=>Special.takeActingLesson(this.state));} actingAudition(){return this.run(()=>Special.auditionActing(this.state));} actingAgent(){return this.run(()=>Special.hireActingAgent(this.state));}
  ...
  sportsJoin(sport:string){return this.run(()=>Special.joinSportsPath(this.state,sport));} sportsTrain(){return this.run(()=>Special.trainSport(this.state));} sportsPro(){return this.run(()=>Special.pursueProSports(this.state));}
  combatTrain(){return this.run(()=>Special.trainCombat(this.state));} combatFight(){return this.run(()=>Special.takeFight(this.state));}
  ...
  royalDuty(){return this.run(()=>Special.royalDuty(this.state));} model(action:'lesson'|'audition'|'photoshoot'|'runway'){return this.run(()=>Special.modelingAction(this.state,action));} race(action:'join'|'train'|'race'){return this.run(()=>Special.racingAction(this.state,action));} directFilm(budget:number){return this.run(()=>Special.directFilm(this.state,budget));}
