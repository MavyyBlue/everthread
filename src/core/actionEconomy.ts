...
export const ACTION_POLICIES = {
  ...
  'credit.application.product': { maxPerAge: 1, message: 'You already applied for that credit product this year.' },
  'credit.purchase.total': { maxPerAge: 8, message: 'You have already made several representative credit purchases this year.' },
  'credit.purchase.account': { maxPerAge: 4, message: 'You have already used that card several times this year. Age up before adding more representative purchases.' },
  ...
  'special.music_business': { maxPerAge: 1, message: 'You already made a major music-business decision this year.' },
  ...
  'special.race': { maxPerAge: 1, message: 'You already committed to this year’s racing season.' },
  'special.racing_contract_seek': { maxPerAge: 1, message: 'You already made a serious racing free-agency inquiry this year.' },
  'special.racing_business': { maxPerAge: 1, message: 'You already made a major racing-contract decision this year.' },
...
function ensureLedger(state: GameState) {
  ...
  if (state.actionLedger.age !== state.character.age) {state.actionLedger.age = state.character.age;state.actionLedger.uses = {};}
  state.actionLedger.lastUsedAge ??= {};state.actionLedger.uses ??={};state.actionLedger.revision=Number.isFinite(state.actionLedger.revision)?state.actionLedger.revision:0;return state.actionLedger;
}
function claimKey(claim: ActionClaim): string {return claim.target ? `${claim.policy}:${claim.target}` : claim.policy;}
export function actionGateStatus(state: GameState, claims: ActionClaim | ActionClaim[]): ActionGateResult {
  const list=Array.isArray(claims)?claims:[claims];const ledger=state.actionLedger;const currentUses=ledger?.age===state.character.age?ledger.uses:{};const lastUsedAge=ledger?.lastUsedAge??{};
  for(const claim of list){const policy:ActionPolicy=ACTION_POLICIES[claim.policy];const key=claimKey(claim);const units=Math.max(1,Math.floor(claim.units??1));if(policy.maxPerAge!==undefined&&(currentUses[key]??0)+units>policy.maxPerAge)return{allowed:false,message:policy.message,blockedPolicy:claim.policy};if(policy.cooldownAges!==undefined){const lastUsed=lastUsedAge[key];if(lastUsed!==undefined&&state.character.age-lastUsed<policy.cooldownAges)return{allowed:false,message:policy.message,blockedPolicy:claim.policy};}}
  return{allowed:true};
}
export function consumeAction(state: GameState, claims: ActionClaim | ActionClaim[]): ActionGateResult {const list=Array.isArray(claims)?claims:[claims];const status=actionGateStatus(state,list);if(!status.allowed)return status;const ledger=ensureLedger(state);for(const claim of list){const key=claimKey(claim);const units=Math.max(1,Math.floor(claim.units??1));ledger.uses[key]=(ledger.uses[key]??0)+units;ledger.lastUsedAge[key]=state.character.age;}ledger.revision+=1;return{allowed:true};}
export function actionAllowed(state: GameState, claims: ActionClaim | ActionClaim[]): boolean {return actionGateStatus(state,claims).allowed;}
export function actionUsesThisAge(state: GameState, policy: ActionPolicyId, target?: string): number {const ledger=state.actionLedger;if(!ledger||ledger.age!==state.character.age)return 0;return ledger.uses[claimKey({policy,target})]??0;}
