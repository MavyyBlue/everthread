...
export function LicenseQuiz({kind,onResult}:{kind:'driving'|'boating'|'pilot';onResult:(r:EngineResult)=>void}){const questions=useMemo(()=>banks[kind],[kind]);const[index,setIndex]=useState(0);const[correct,setCorrect]=useState(0);const done=index>=questions.length;const q=questions[index];
  if(done)return <div className="quiz-result"><strong>Quiz complete</strong><p>{correct}/{questions.length} safe decisions.</p><button onClick={()=>onResult(gameEngine.license(kind,Math.round(correct/questions.length*100)))}>Submit test</button></div>;
