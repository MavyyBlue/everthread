...
import { isSecretYukiNpc } from '../systems/SecretCodeSystem';
    ...
    >
      <div className="threadspace-world" style={worldStyle}>
        ...
        {renderedNodes.map(node=>{
          ...
          const person=node.person!;const rel=state.relationships.find(item=>item.npcId===person.id);const memberships=person.memberships.filter(item=>expandedFolders.includes(item.folderId)&&visibleFolders.includes(item.folderId));const connectionLabels=labelsByPersonId.get(person.id)??[];const npc=state.npcs[person.id];const portraitMode=npc?npcPortraitRevealMode(state,npc.id):'silhouette';const appearance=npc&&portraitMode==='portrait'?projectNpcAppearance(state,npc):undefined;const secretYuki=isSecretYukiNpc(state,npc);
          return <Fragment key={node.id}>
            <button className={`threadspace-node threadspace-node--person ${!person.alive?'deceased':''} ${secretYuki?'threadspace-node--secret-yuki':''}`} style={{left:node.x,top:node.y}} onClick={()=>onSelect(person.id)}>
              <span className="threadspace-person-mark">{appearance?<CharacterPortrait appearance={appearance} age={person.age} size={37} label={`${person.name} portrait`} fallback={<span aria-hidden="true">{person.name[0]}</span>}/>:<CharacterSilhouette age={person.age} size={37} frame="none" label={`${person.name} silhouette`}/>}</span>{secretYuki&&<span className="threadspace-secret-yuki-mark" aria-hidden="true">✦</span>}{!person.alive&&<img className="threadspace-deceased-stamp" src={EVERTHREAD_UI_ICONS.deceased} alt="" aria-hidden="true"/>}<div className="threadspace-person-copy"><strong>{person.name}</strong><small>{rel?relationshipTypeLabel(rel.type):'connection'} · age {person.age}{!person.alive?' · deceased':''}</small>{memberships.length>1&&<em>{memberships.length} circles</em>}</div><b>{Math.round(person.relationshipScore)}</b>
            </button>
