...
export function EstatePlanningPanel({state,onResult}:{state:GameState;onResult:(result:EngineResult)=>void}){
  ...
  return <div className="stack">
    <section className="hero-card"><p className="eyebrow">Phase 5 · estate plan</p><h2>{formatMoney(preview.distributableValue)} estimated for family heirs</h2><p>Estate obligations are settled first. By default, a surviving spouse receives half of the residuary estate and living children divide the other half. You can explicitly change those shares below.</p><div className="finance-grid"><div><small>Estimated estate</small><strong>{formatMoney(preview.estateValue)}</strong></div><div><small>Estate obligations</small><strong>{formatMoney(preview.estateObligations)}</strong></div><div><small>Eligible heirs</small><strong>{heirs.length}</strong></div><div><small>Likely forced sales</small><strong>{preview.forcedSaleIds.length}</strong></div></div></section>
