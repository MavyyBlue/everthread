...
function PaymentsView({state,onResult,onBack}:{state:GameState;onResult:(result:EngineResult)=>void;onBack:()=>void}){
  ...
  return <>
    ...
    <section className="hero-card banking-payments-hero"><p className="eyebrow">Bills & Payments</p><h2>{summary.pastDue>0?`${money(summary.pastDue)} past due`:`${money(summary.dueNow)} due`}</h2><p>Manage annual minimum card payments and financed home or vehicle bills here. Auto-pay uses available cash when you Age Up.</p></section>
    ...
    </section>)}</div>:<section className="action-card"><h3>No active bills</h3><p className="muted">Financed homes, financed vehicles, and credit-card balances will appear here when they have an active obligation.</p></section>}
