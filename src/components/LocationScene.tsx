...
import { LOCATION_SCENE_ACTIONS, locationSceneDefinition, type LocationSceneActionId, type LocationSceneBankActionId, type LocationSceneMotorsActionId, type LocationSceneRealtyActionId, type LocationSceneResidentialActionId, type LocationSceneGroupDefinition, type LocationScenePlaceId } from '../data/locationScenes';
...
type DetailState=
  ...
  |{kind:'companion';actionId:'shared.park.walk'|'shared.park.play'|'date.park'|'date.home'}
...
function placeIcon(placeId:LocationScenePlaceId){return placeId==='weaver-park'?'park' as const:placeId==='central-everthread-bank'?'bank' as const:placeId==='loomline-motors'?'car' as const:placeId==='hearthline-realty'?'key' as const:placeId==='threadwell-residential'?'home' as const:'music' as const;}
...
export function LocationScene({state,placeId,onClose,onResult}:{state:GameState;placeId:LocationScenePlaceId;onClose:()=>void;onResult:(result:EngineResult)=>void}){
  ...
  const chooseAction=(actionId:LocationSceneActionId)=>{
    ...
    if(locationSceneCompanionPlan(actionId)){setDetail({kind:'companion',actionId:actionId as 'shared.park.walk'|'shared.park.play'|'date.park'|'date.home'});return;}
  ...
  const sheetTitle=detail?.kind==='confirm'?'Confirm choice':detail?.kind==='companion'?LOCATION_SCENE_ACTIONS[detail.actionId].label:detail?.kind==='catalog'?'Your music':detail?.kind==='partnership'?'Distribution offers':detail?.kind==='bank'||detail?.kind==='motors'||detail?.kind==='realty'||detail?.kind==='residential'?LOCATION_SCENE_ACTIONS[detail.actionId].label:selectedGroupId==='__all'?'Things to do':selectedGroup?.label??scene.label;
  ...
  return <section ref={sceneRef} className="location-scene" aria-label={scene.label}>
    ...
    <BottomSheet open={Boolean(selectedGroupId)} title={sheetTitle} onClose={closeSheet} wide={detail?.kind==='bank'||detail?.kind==='motors'||detail?.kind==='realty'||detail?.kind==='residential'}>
