...
export function postContent(state:GameState,platform:keyof typeof platformNames):EngineResult {
  if(state.character.age<13)return{success:false,messages:[{text:'Social posting becomes available in the teen years.'}]};const rng=createRng(`${state.seed}-social`,state.rngCounter);const creativity=state.character.secondary.creativity;const charisma=state.character.secondary.charisma;const quality=(creativity+charisma)/2+rng.int(-25,25);const base=Math.max(4,Math.round(quality*quality/20));const current=state.fame.platforms[platform]??0;const viral=rng.chance(clamp((quality-72)/180,0,.12));const gained=Math.round(base*(viral?rng.int(30,120):rng.int(1,5)));
...
export function fameActivity(state:GameState,activity:'interview'|'commercial'|'book'|'event'|'endorsement'|'respond_fans'):EngineResult {
  if(state.fame.fame<12)return{success:false,messages:[{text:'You are not famous enough for that public opportunity yet.'}]};const rng=createRng(`${state.seed}-publicity`,state.rngCounter);let pay=0;let fame=0;let rep=0;
