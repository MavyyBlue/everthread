...
export function processFameYear(state:GameState) {
  ...
  if(state.fame.fame>0&&rng.chance(clamp(.02+state.fame.fame/1500+world.fameScandalChanceDelta,0,.25))){
  ...
  const organic=Math.round(state.fame.followers*(state.fame.engagement/100)*rng.int(1,7)/100*world.fameOrganicGrowthMultiplier);state.fame.followers=Math.max(0,state.fame.followers+organic-Math.round(state.fame.followers*(state.fame.engagement<20?.04:.01)));
  state.rngCounter=rng.counter();
}

export function postContent(state:GameState,platform:keyof typeof platformNames):EngineResult {
  if(state.character.age<SOCIAL_MEDIA_MIN_AGE)return{success:false,messages:[{text:'Social posting becomes available in the teen years.'}]};const gate=consumeAction(state,[{policy:'fame.post.total'},{policy:'fame.post.platform',target:platform}]);if(!gate.allowed)return{success:false,messages:[{text:gate.message!}]};const rng=createRng(`${state.seed}-social`,state.rngCounter);const creativity=state.character.secondary.creativity;const charisma=state.character.secondary.charisma;const quality=(creativity+charisma)/2+rng.int(-25,25);const base=Math.max(4,Math.round(quality*quality/20));const current=state.fame.platforms[platform]??0;const viral=rng.chance(clamp((quality-72)/180,0,.12));const gained=Math.round(base*(viral?rng.int(30,120):rng.int(1,5)));
...
export function fameActivity(state:GameState,activity:'interview'|'commercial'|'book'|'event'|'endorsement'|'respond_fans'):EngineResult {
  ...
  if(activity==='commercial'){pay=Math.round(2500*state.fame.fame*rng.int(5,16)/10*worldConditionModifiers(state).publicityPayMultiplier);fame=2;}
  if(activity==='book'){pay=Math.round(1800*state.fame.fame*rng.int(6,18)/10*worldConditionModifiers(state).publicityPayMultiplier);fame=3;rep=2;}
  if(activity==='event'){fame=2;rep=2;state.finances.cash-=300;}
  if(activity==='endorsement'){pay=Math.round(3000*state.fame.fame*rng.int(8,22)/10*worldConditionModifiers(state).publicityPayMultiplier);fame=3;rep=rng.int(-1,2);}
