...
import { getSecuredLoanStatus, payPersonalLoanBill, paySecuredLoanBill, scheduledLoanPaymentAmount } from './FinanceSystem';
import { creditInstitutionById as institutionById } from '../data/creditInstitutions';
import { personalLoanProductById } from '../data/personalLoans';

export type PaymentObligationKind='credit_card'|'mortgage'|'vehicle'|'personal_loan';
...
export function getPaymentObligations(state:GameState):PaymentObligation[]{
  ...
  const personal=state.finances.liabilities.filter(loan=>loan.kind==='personal').map(loan=>{const delinquency=loan.delinquency;const pastDue=delinquency?.status==='delinquent'?roundMoney(Math.min(loan.balance,Math.max(0,delinquency.arrears))):0;const paidAhead=Number.isFinite(loan.prepaidThroughAge)&&Number(loan.prepaidThroughAge)>=state.character.age+1;const dueNow=pastDue>0?pastDue:paidAhead?0:scheduledLoanPaymentAmount(loan);const product=loan.productId?personalLoanProductById[loan.productId]:undefined;const institution=loan.institutionId?institutionById[loan.institutionId]?.name:undefined;return{id:loanObligationId(loan.id),kind:'personal_loan' as const,sourceId:loan.id,title:product?.name??(loan.origin==='deficiency'?'Unsecured deficiency debt':loan.origin==='hardship'?'Hardship debt':'Personal loan'),subtitle:`${institution?`${institution} · `:''}${(loan.annualRate*100).toFixed(1)}% APR`,balance:roundMoney(loan.balance),dueNow,pastDue,autoPay:loan.autoPay!==false,status:pastDue>.5?'past_due' as const:paidAhead?'paid_ahead' as const:'current' as const};});
  return[...secured,...personal,...cards].sort((a,b)=>Number(b.status==='past_due')-Number(a.status==='past_due')||b.dueNow-a.dueNow||a.title.localeCompare(b.title));
}
...
export function setPaymentAutoPay(state:GameState,obligationId:string,enabled:boolean):EngineResult{
  ...
  if(obligationId.startsWith('loan:')){const id=obligationId.slice(5);const loan=state.finances.liabilities.find(item=>item.id===id&&(item.kind==='mortgage'||item.kind==='car'||item.kind==='personal'));if(!loan)return{success:false,messages:[{text:'That financing bill is no longer active.'}]};loan.autoPay=enabled;const label=loan.kind==='personal'?(loan.productId?personalLoanProductById[loan.productId]?.name:'personal debt'):(getSecuredLoanStatus(state,loan)?.collateralName??'this financed asset');return{success:true,stateChanges:['paymentPreference'],messages:[{text:`Annual auto-pay is now ${enabled?'on':'off'} for ${label}.`}]};}
...
export function payPaymentObligation(state:GameState,obligationId:string):EngineResult{
  ...
  if(obligation.kind==='personal_loan')return payPersonalLoanBill(state,obligation.sourceId);
  return paySecuredLoanBill(state,obligation.sourceId);
}

export function migratePaymentState(state:GameState){
  for(const loan of state.finances?.liabilities??[]){if(loan.kind!=='mortgage'&&loan.kind!=='car'&&loan.kind!=='personal')continue;loan.autoPay=loan.autoPay!==false;if(!Number.isFinite(loan.prepaidThroughAge))delete loan.prepaidThroughAge;else loan.prepaidThroughAge=Math.floor(Number(loan.prepaidThroughAge));}
