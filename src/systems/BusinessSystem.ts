...
export function addBusinessProduct(state:GameState,businessId:string):EngineResult {
  ...
  b.capital-=cost;b.productIds.push(`${b.industryId}_product_${b.productIds.length+1}`);b.demand=clamp(b.demand+6);scheduleBusinessProductStory(state,b.id);return{success:true,messages:[{text:`${b.name} launched ${industry.productNames[b.productIds.length-1]}.`}]};
}
