...
function buildEstatePlan(state:GameState):EstatePlan{
  ...
  for(const item of unassigned){const ranked=[...heirs].sort((a,b)=>((targetById.get(b.npc.id)??0)-(assignedValue.get(b.npc.id)??0))-((targetById.get(a.npc.id)??0)-(assignedValue.get(a.npc.id)??0)));const heir=ranked[0]!;const remaining=Math.max(0,(targetById.get(heir.npc.id)??0)-(assignedValue.get(heir.npc.id)??0));if(heirs.length>1&&item.value>remaining*1.10){liquid+=itemSaleValue(state,item);forcedSaleIds.push(`${item.kind}:${item.id}`);continue;}assignedItems.get(heir.npc.id)!.push(item);assignedValue.set(heir.npc.id,(assignedValue.get(heir.npc.id)??0)+item.value);}
