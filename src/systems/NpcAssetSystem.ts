...
export function playerBusinessFromNpcHolding(state:GameState,business:NpcBusinessHolding):Business {
  const years=Math.max(0,state.currentYear-business.foundedYear);const foundedAge=Math.max(18,state.character.age-years);const profit=Math.round(business.annualProfit);const revenue=Math.max(Math.abs(profit)*3,Math.round(business.valuation*.28),1);return{id:business.id,industryId:business.industryId,name:business.name,foundedAge,...(business.origin==='inherited'?{origin:'inherited' as const,...(business.inheritedFromNpcId?{inheritedFromNpcId:business.inheritedFromNpcId}:{})}:{}),countryId:business.countryId??state.character.countryId,city:business.city??state.character.city,capital:Math.round(business.valuation*.32),revenue,expenses:Math.max(0,revenue-profit),profit,employees:Math.max(1,Math.floor(business.employees)),demand:clamp(50+(state.economy.businessDemandIndex-1)*35),reputation:clamp(business.reputation),valuation:safeMoney(business.valuation),productIds:[],priceIndex:1,marketingBudget:0,compensationIndex:1,bankrupt:!business.active};
}
