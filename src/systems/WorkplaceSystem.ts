...
import { scheduleWorkplaceConcernStory, scheduleWorkplaceFeedbackStory } from './SystemicStorySystem';
...
function createWorkNpc(state:GameState,role:SocialWorldMemberRole,worldKey:string,ordinal:number,record:CareerRecord,industry:string,rng:ReturnType<typeof createRng>,used:Set<string>):Npc{
  ...
  assignGeneratedNpcOrientation(state,npc);state.npcs[id]=npc;ensureNpcLife(state,npc);addWorkRelationship(state,npc,role,Math.max(0,elapsed),rng);return npc;
}
...
export function reportCoworkerIssue(state:GameState,npcId:string):EngineResult{
  ...
  state.rngCounter=rng.counter();scheduleWorkplaceConcernStory(state,info.world.id,target.id);return{success:true,messages:[{text:accepted?'Your concern is taken seriously and handled through the workplace process.':'The concern does not resolve cleanly, and workplace tension rises.'}]};
}
