...
export function acceptAssetFinanceOffer(state:GameState,rawRequest:AssetFinanceRequest,offerId:string):AssetFinanceApplication {
  ...
  const loan:Loan={id:makeStateId(state,'loan'),kind:request.kind==='home'?'mortgage':'car',principal:offer.amountFinanced,balance:offer.amountFinanced,annualRate:offer.annualRate,annualPayment:offer.annualPayment,remainingYears:offer.termYears};
