...
const systemEventById:Record<string,GameEventDefinition>={
  financial_independence_transition:{id:'financial_independence_transition',category:'money',title:'Your Money, Your Responsibility',descriptions:['Adulthood changes what belongs to you financially.'],minAge:18,maxAge:18,probability:0,cooldown:99,tags:['money','milestone'],choices:[{id:'stay_supported',label:'Stay with family for now',effects:{flags:{financiallyIndependent:false,financialSupportChoiceMade:true}}},{id:'become_independent',label:'Become financially independent',effects:{flags:{financiallyIndependent:true,financialSupportChoiceMade:true}}}]},
...
export function resolvePendingEvent(state:GameState,choiceId:string):EngineResult{
  ...
  const rng=createRng(`${state.seed}-choice`,state.rngCounter);const baseContext:EffectContext={eventTitle:def.title,choiceLabel:choice.label};applyEffect(state,choice.effects,rng,pending.payload,baseContext);let outcomeText='';
