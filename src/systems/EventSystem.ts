...
import { systemicConsequenceEventById } from '../data/systemicConsequenceEvents';
import type { ChoiceEffect, EngineResult, GameEventDefinition, GameState, PendingConsequenceContext, PendingEvent, Relationship, TimelineCategory } from '../types/game';
...
import { bankruptcyPreview, reduceHardshipDebtWithInvestments } from './FinanceSystem';
import { cancelConsequence, cancelPendingConsequence, completePendingConsequence, eventLastTriggeredAge, pendingContextFromConsequence, recordEventTriggeredAge, scheduleConsequence, takeNextDueConsequence } from './ConsequenceSystem';

const systemEventById:Record<string,GameEventDefinition>={
  financial_independence_transition:{id:'financial_independence_transition',category:'money',title:'Your Money, Your Responsibility',descriptions:['You are legally an adult, but becoming financially independent is a separate life transition. While you remain in your family household, ordinary living costs stay with your supporting household. You can take responsibility for your own costs now or remain supported for the time being.'],minAge:18,maxAge:18,probability:0,cooldown:99,tags:['money','milestone'],choices:[{id:'stay_supported',label:'Stay with family for now',effects:{flags:{financiallyIndependent:false,financialSupportChoiceMade:true}}},{id:'become_independent',label:'Become financially independent',effects:{flags:{financiallyIndependent:true,financialSupportChoiceMade:true}}}]},
  financial_pressure_notice:{id:'financial_pressure_notice',category:'money',title:'Financial Pressure',descriptions:['Your annual costs exceeded the money you had available, creating hardship debt.'],minAge:18,maxAge:110,probability:0,cooldown:99,tags:['money','financial-crisis'],choices:[{id:'reduce',label:'Use investments to reduce debt'},{id:'review',label:'Review bankruptcy options',effects:{flags:{bankruptcyReviewRequested:true}}},{id:'carry',label:'Carry the debt for now'}]},
};
...
type EffectContext={eventId:string;eventTitle:string;choiceLabel:string;outcomeText?:string;chainId?:string};
...
function eligible(state:GameState,event:GameEventDefinition){
  ...
  const lastAge=eventLastTriggeredAge(state,event.id);if(lastAge!==undefined&&age-lastAge<=event.cooldown)return false;
  return true;
}

export function eventEligibleForState(state:GameState,event:GameEventDefinition){return eligible(state,event);}
function eventDefinition(eventId:string){return eventById[eventId]??specialCareerStoryEventById[eventId]??systemicConsequenceEventById[eventId]??systemEventById[eventId];}
function eventChoices(event:GameEventDefinition){return coherentEventChoices(event);}
function toPending(event:GameEventDefinition,description:string,payload?:Record<string,unknown>,consequence?:PendingConsequenceContext):PendingEvent{return{eventId:event.id,title:event.title,description,choices:eventChoices(event).map(c=>({id:c.id,label:c.label})),...(payload?{payload}:{}),...(consequence?{consequence}:{})};}
function renderDescription(state:GameState,event:GameEventDefinition,rng:ReturnType<typeof createRng>,payload?:Record<string,unknown>){let description=rng.pick(event.descriptions);if(event.id==='financial_pressure_notice'){const shortfall=Number(payload?.shortfall??0),debt=Number(payload?.debt??0);if(shortfall>0)description=`Your annual costs exceeded the money you had available. ${Math.round(shortfall).toLocaleString()} became hardship debt${debt>0?`, bringing unsecured personal debt to ${Math.round(debt).toLocaleString()}`:''}. Nothing will be corrected automatically—you decide how to respond.`;}const npcId=typeof payload?.npcId==='string'?payload.npcId:undefined;const npc=npcId?state.npcs[npcId]:undefined;const npcName=npc?`${npc.firstName} ${npc.lastName}`:'someone close to you';if(event.id==='late_life_reunion'&&npc)description=`${npcName} reaches out with a simple message: “I was thinking about you.” The connection carries more history than the message says.`;const worldId=typeof payload?.worldId==='string'?payload.worldId:undefined;const world=worldId?state.socialWorlds.find(item=>item.id===worldId):undefined;description=description.replace(/\{NPC_NAME\}/g,npcName).replace(/\{NPC_FIRST\}/g,npc?.firstName??'someone').replace(/\{ORIGIN_AGE\}/g,String(payload?.originAge??state.character.age)).replace(/\{WORLD_NAME\}/g,world?.name??'your school');if(npc&&isProceduralScenario(event)){if(event.category==='friends')description=description.replace(/your friend/i,npcName).replace(/a friend/i,npcName);if(event.category==='family')description=description.replace(/someone in the family/i,npcName).replace(/a relative/i,npcName).replace(/someone/i,npcName);if(event.category==='romance')description=description.replace(/someone you like/i,npcName);if(event.category==='work')description=description.replace(/your boss/i,npcName).replace(/a coworker/i,npcName).replace(/someone/i,npcName);if(event.category==='school')description=description.replace(/a classmate/i,npcName).replace(/someone/i,npcName);}return description;}
...
export function processDelayedEvents(state:GameState):PendingEvent|undefined{
  ...
  while(true){
    const due=takeNextDueConsequence(state);if(!due)return undefined;
    const event=eventDefinition(due.eventId);if(!event){cancelConsequence(state,due,'event_definition_missing');continue;}
    const rng=createRng(`${state.seed}-event-text`,state.rngCounter);const description=renderDescription(state,event,rng,due.payload);state.rngCounter=rng.counter();
    return toPending(event,description,due.payload,pendingContextFromConsequence(due));
  }
}
...
function applyEffect(state:GameState,effect:ChoiceEffect|undefined,rng:ReturnType<typeof createRng>,payload:Record<string,unknown>|undefined,context:EffectContext){if(!effect)return;let selectedRelationship:Relationship|undefined;
  ...
  if(effect.school){const payloadWorldId=typeof payload?.worldId==='string'?payload.worldId:undefined;const world=payloadWorldId?state.socialWorlds.find(item=>item.id===payloadWorldId):undefined;if(world?.school){for(const[key,value]of Object.entries(effect.school))if(value!==undefined){const obj=world.school as unknown as Record<string,number|string|undefined>;const current=Number(obj[key]??0);obj[key]=clamp(current+Number(value));}}}
  if(effect.schedule){const target=effect.schedule.npcSelector==='effect'?selectedRelationship:effect.schedule.npcSelector?selectRelationshipTarget(state,effect.schedule.npcSelector,rng,payload):undefined;if(!effect.schedule.npcSelector||target){const targetId=target?.npcId;const chainId=context.chainId??`event:${context.eventId}:${state.character.age}:${targetId??'none'}`;const requiredRelationshipTypes=effect.schedule.requiredRelationshipTypes;const scheduledPayload={originAge:state.character.age,...(targetId?{npcId:targetId}:{}),...(requiredRelationshipTypes?{requiredRelationshipTypes}:{})};scheduleConsequence(state,{eventId:effect.schedule.eventId,dueAge:state.character.age+Math.max(1,effect.schedule.years),payload:scheduledPayload,priority:effect.schedule.eventId.startsWith('special_career_')?'high':'normal',chainId,origin:{kind:'event',id:context.eventId,age:state.character.age},...(targetId?{targetRefs:[{kind:'npc' as const,id:targetId}],validity:{targetMustExist:true,targetMustBeAlive:true,...(requiredRelationshipTypes?{requiredRelationshipTypes}:{})}}:{}),dedupeKey:`followup:${effect.schedule.eventId}:${targetId??'-'}:${chainId}`});}}
}
...
export function resolvePendingEvent(state:GameState,choiceId:string):EngineResult{
  const pending=state.pendingEvent;if(!pending)return{success:false,messages:[{text:'There is no unresolved event.'}]};const def=eventDefinition(pending.eventId);if(!def){cancelPendingConsequence(state,pending.consequence,'event_definition_missing_at_resolution');state.pendingEvent=undefined;return{success:false,messages:[{text:'The event definition could not be loaded.'}]};}const coherentChoices=eventChoices(def);const coherentChoice=coherentChoices.find(c=>c.id===choiceId);const legacyChoice=!coherentChoice&&pending.choices.some(item=>item.id===choiceId)?def.choices.find(c=>c.id===choiceId):undefined;const legacyIndex=!coherentChoice&&!legacyChoice?def.choices.findIndex(c=>c.id===choiceId):-1;const compatibilityChoice=legacyIndex>=0?coherentChoices[legacyIndex]:undefined;const choice=coherentChoice??legacyChoice??compatibilityChoice;if(!choice)return{success:false,messages:[{text:'That choice is not available.'}]};
  const targetNpcId=typeof pending.payload?.npcId==='string'?pending.payload.npcId:undefined;const targetRel=targetNpcId?state.relationships.find(rel=>rel.npcId===targetNpcId):undefined;const targetWorldId=typeof pending.payload?.worldId==='string'?pending.payload.worldId:undefined;const targetSchool=targetWorldId?state.socialWorlds.find(world=>world.id===targetWorldId)?.school:undefined;const targetSchoolBefore=targetSchool?{conduct:targetSchool.conduct,socialStanding:targetSchool.socialStanding,attendance:targetSchool.attendance}:undefined;const cashBefore=state.finances.cash;const relationshipBefore=targetRel?.score;const workBefore=state.employment.current?.performance;const schoolBefore=activeEducationRecord(state)?.performance;const healthBefore=state.character.stats.health;const happinessBefore=state.character.stats.happiness;const stressBefore=state.character.secondary.stress;const fameBefore=state.fame.fame;const publicReputationBefore=state.fame.publicReputation;const legalHeatBefore=state.legal.investigationHeat;
  const rng=createRng(`${state.seed}-choice`,state.rngCounter);const baseContext:EffectContext={eventId:def.id,eventTitle:def.title,choiceLabel:choice.label,chainId:pending.consequence?.chainId};applyEffect(state,choice.effects,rng,pending.payload,baseContext);let outcomeText='';
  if(def.id==='financial_pressure_notice'){if(choice.id==='reduce'){const paid=reduceHardshipDebtWithInvestments(state);outcomeText=paid>0?`You reduced unsecured debt by ${Math.round(paid).toLocaleString()}.`:'You do not have available investments or cash to reduce this debt right now.';}else if(choice.id==='review'){const preview=bankruptcyPreview(state);outcomeText=preview.eligible?'Bankruptcy is currently available in Credit & Banking if you decide to file.':'You reviewed bankruptcy, but you are not currently eligible to file.';}else outcomeText='You chose to carry the debt for now. Future costs and missed obligations can make the situation worse.';}if(choice.outcomes?.length){const outcome=rng.weighted(choice.outcomes.map(o=>({item:o,weight:o.weight})));outcomeText=outcome.text;applyEffect(state,outcome.effects,rng,pending.payload,{...baseContext,outcomeText});}
  ...
  state.timeline.push({id:makeStateId(state,'timeline'),year:state.currentYear,age:state.character.age,category:timelineCategory(def.category),importance:2,title:def.title,text:`${pending.description} ${summary}`,...(moneyDelta?{moneyDelta}:{}),...(relationshipDelta?{relationshipDelta}:{}),...(targetNpcId?{npcIds:[targetNpcId]}:{})});completePendingConsequence(state,pending.consequence);state.pendingEvent=undefined;state.rngCounter=rng.counter();
  const stateChanges:string[]=[];if(moneyDelta)stateChanges.push(`cash ${moneyDelta>0?'+':''}${Math.round(moneyDelta)}`);if(relationshipDelta&&targetNpcId)stateChanges.push(`relationship:${targetNpcId} ${relationshipDelta>0?'+':''}${relationshipDelta}`);if(workBefore!==workAfter&&workAfter!==undefined)stateChanges.push(`workPerformance ${workAfter}`);if(schoolBefore!==schoolAfter&&schoolAfter!==undefined)stateChanges.push(`academicPerformance ${schoolAfter}`);if(targetSchool&&targetSchoolBefore&&targetWorldId){if(targetSchoolBefore.conduct!==targetSchool.conduct)stateChanges.push(`schoolConduct:${targetWorldId} ${targetSchool.conduct}`);if(targetSchoolBefore.socialStanding!==targetSchool.socialStanding)stateChanges.push(`schoolSocialStanding:${targetWorldId} ${targetSchool.socialStanding}`);if(targetSchoolBefore.attendance!==targetSchool.attendance)stateChanges.push(`schoolAttendance:${targetWorldId} ${targetSchool.attendance}`);}if(healthBefore!==state.character.stats.health)stateChanges.push(`health ${state.character.stats.health}`);if(happinessBefore!==state.character.stats.happiness)stateChanges.push(`happiness ${state.character.stats.happiness}`);if(stressBefore!==state.character.secondary.stress)stateChanges.push(`stress ${state.character.secondary.stress}`);if(fameBefore!==state.fame.fame)stateChanges.push(`fame ${state.fame.fame}`);if(publicReputationBefore!==state.fame.publicReputation)stateChanges.push(`publicReputation ${state.fame.publicReputation}`);if(legalHeatBefore!==state.legal.investigationHeat)stateChanges.push(`legalHeat ${state.legal.investigationHeat}`);
