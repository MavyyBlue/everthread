...
export function processEconomyYear(state:GameState) {
  ...
  const costShock=(rng.int(-2,4)+(recession?1:0))/100+world.inflationRateDelta;
  const salaryShock=costShock*.55+rng.int(-1,2)/100+(boom?.015:recession?-.018:0)+world.salaryGrowthRateDelta;
  const housingShock=costShock*.45+rng.int(-3,4)/100+(boom?.045:recession?-.065:0)+world.housingGrowthRateDelta;
  ...
  state.economy.businessDemandIndex=clamp(state.economy.businessDemandIndex*(1+(boom?.06:recession?-.07:rng.int(-2,3)/100)+world.businessDemandGrowthRateDelta)+(1-state.economy.businessDemandIndex)*.04,.60,1.70);
